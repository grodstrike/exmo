...
ORDER_LIFE_TIME = 0.5 # через сколько минут отменять неисполненный ордер на покупку CURRENCY_1
...
# Использование MACD для принятия решения об открытии торгов
def should_buy(pair):
        ...
        if sell_orders: # открытые ордера есть
            ...
            if float(balances[CURRENCY_1]) >= CURRENCY_1_MIN_QUANTITY: # Есть ли в наличии CURRENCY_1, которую можно продать?
                ...
                if float(balances[CURRENCY_2]) >= CAN_SPEND:
                    if should_buy(CURRENT_PAIR): # Если растет рынок
                        ...
                        raise ScriptQuitCondition('Выход, по MACD рынок падает')
                else:
